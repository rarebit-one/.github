...
#
# Ported from sidekick-labs/.github, where it was written and first run. The
# dated observations below are the sidekick-labs incidents that motivated each
# branch; the mechanism is org-agnostic and driven entirely by $ORG.
#
...
#   "require branches to be up to date", an armed
#   PR that falls BEHIND stalls forever: fully green, mergeable, and going
#   nowhere. Observed in sidekick-labs 2026-08-03: three companion-kit PRs (#427, #428, #433) each
...
#   sidekick-labs/companion-kit#370's head `a4d85fc` (pushed 2026-07-28, and the commit that
...
# check-runs. Either alone gives false positives — sidekick-labs' `skl_glasses_mobile` has 0
...
EOF2
  ...
  # read is therefore a coin flip — observed in sidekick-labs 2026-08-03: claude-plugins#6 landed
...
EOF3
  ...
  # its own bucket. (sidekick-labs/product-brain#250 was the case that exposed this.)
